...
myfun=function (x, xlab, ylab, xlim, ylim,legend = TRUE, subtitles = TRUE,
                cex.subtitles = 0.75, riskdist = TRUE, scat1d.opts = list(nhistSpike = 200),group=TRUE, 
                ...) 
{
  at <- attributes(x)
  if (missing(ylab)) 
  ...
  if (!(is.logical(legend) && !legend)) {
    if (is.logical(legend)) 
      legend <- list(x = 0.75, y = 0.6)
    legend(legend, c("Apparent", "Bias-corrected", 
                     "Ideal"), lty = c(3, 1, 2), lwd=c(1,1,1),bty = "n",cex=0.7) # 0.5
  }
  invisible(err)
}